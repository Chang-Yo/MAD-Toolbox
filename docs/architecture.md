...
- **事件通道支持 feature 自定义载荷。** 交互式作业（BBDown 扫码登录：运行中需向用户呈现二维码、等待扫码）既非查询（时长不有界）也非普通作业（运行中要向 UI 递交内容），不为其单设第三类通路——解析器发射自定义事件（如 `qr-ready: <path>`），发起页面订阅消费。该扩展点必须在信封/事件设计中预留，否则实现期会被迫开后门。
...
| 事项 | 现状 |
|---|---|
| command / 事件清单的完整枚举 | 实施第 1 步（契约）时定稿 |
