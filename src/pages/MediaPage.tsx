...
export function MediaPage({
  ...
  const selectOperation = (operation: MediaOptions["operation"]) => {
    const suggestedContainer: Partial<Record<MediaOptions["operation"], MediaOptions["container"]>> = {
      remux: "mkv",
      transcode: "mov",
      "video-extract": "mkv",
      audio: "m4a",
  ...
  const videoEncoderFallback = [
    ...
    "h264_qsv",
    "mpeg4"
  ].find((encoder) => ffmpegEncoders.includes(encoder)) as
    ...
    <div className="page">
          ...
          {options.inputs.map((path) => (
            <div className="file-row" key={path}>
              ...
              <button
                type="button"
                title="移除"
                onClick={() => update("inputs", options.inputs.filter((item) => item !== path))}
              >
                <X size={14} />
              </button>
            </div>
      ...
      <section className="tool-panel">
        ...
        {options.operation === "subtitle-extract" && (
          <div className="notice warning">
            ...
            <div>
              ...
              <p>可导入、识别并导出 SRT/ASS；PGS、VobSub 等图片字幕需要 OCR，FFmpeg 无法直接转成文字字幕。</p>
            </div>
          </div>
        )}

        <div className="form-grid single-leading">
          <Field label="输出目录" hint={`留空时输出到原文件目录；可直接输入或在${fileManagerName}中选择。`}>
            <DirectoryInput
        ...
        {options.operation !== "pr-compatible" && (
          ...
          <Field label={options.operation === "subtitle-extract" ? "字幕输出格式" : "输出封装"}>
            <SelectInput
              value={options.container}
              onChange={(event) => update("container", event.target.value as MediaOptions["container"])}
          ...
          <Field label="开始时间" hint="例如 00:01:23.500">
            <TextInput value={options.startTime} onChange={(event) => update("startTime", event.target.value)} />
          </Field>
          <Field label="持续时间" hint="留空处理到文件末尾">
            <TextInput value={options.duration} onChange={(event) => update("duration", event.target.value)} />
          </Field>
          {pageMode === "media" && (
            <Field label="播放速度" hint="0.5–2.0，1 表示原速">
              <TextInput type="number" min={0.5} max={2} step={0.05} value={options.speed} onChange={(e) => update("speed", Number(e.target.value))} />
            </Field>
          )}
            </div>
          </section>
          ...
          {pageMode === "media" && !["audio", "gif", "frames", "thumbnail"].includes(options.operation) && (
          <section className="parameter-section">
            <h3>视频编码与质量</h3>
            <div className="form-grid">
          <Field label="视频编码">
            <SelectInput
              ...
              onChange={(event) => update("videoCodec", event.target.value as MediaOptions["videoCodec"])}
            >
              <option value="copy">复制原视频流</option>
              {ffmpegEncoders.includes("prores_ks") && <option value="prores_ks">Apple ProRes 422</option>}
              {ffmpegEncoders.includes("h264_videotoolbox") && <option value="h264_videotoolbox">H.264 VideoToolbox</option>}
              {ffmpegEncoders.includes("hevc_videotoolbox") && <option value="hevc_videotoolbox">HEVC VideoToolbox</option>}
              {ffmpegEncoders.includes("h264_amf") && <option value="h264_amf">H.264 AMD AMF</option>}
              {ffmpegEncoders.includes("hevc_amf") && <option value="hevc_amf">HEVC AMD AMF</option>}
              {ffmpegEncoders.includes("h264_nvenc") && <option value="h264_nvenc">H.264 NVIDIA NVENC</option>}
              {ffmpegEncoders.includes("hevc_nvenc") && <option value="hevc_nvenc">HEVC NVIDIA NVENC</option>}
              {ffmpegEncoders.includes("h264_qsv") && <option value="h264_qsv">H.264 Intel QSV</option>}
              {ffmpegEncoders.includes("hevc_qsv") && <option value="hevc_qsv">HEVC Intel QSV</option>}
              {ffmpegEncoders.includes("libopenh264") && <option value="libopenh264">H.264 OpenH264 软件编码</option>}
              {ffmpegEncoders.includes("libx264") && <option value="libx264">H.264 软件编码</option>}
              {ffmpegEncoders.includes("libx265") && <option value="libx265">HEVC 软件编码</option>}
              {ffmpegEncoders.includes("mpeg4") && <option value="mpeg4">MPEG-4 Part 2 软件编码</option>}
              {ffmpegEncoders.includes("libvpx-vp9") && <option value="libvpx-vp9">VP9</option>}
              {ffmpegEncoders.includes("libsvtav1") && <option value="libsvtav1">AV1 (SVT-AV1)</option>}
            </SelectInput>
          </Field>
          <Field label="视频码率" hint="例如 8M、2500k；留空自动">
            <TextInput value={options.videoBitrate} onChange={(e) => update("videoBitrate", e.target.value)} placeholder="8M" />
          </Field>
          <Field label="恒定质量 CRF" hint="仅软件编码有效；数值越小质量越高">
            <TextInput type="number" min={0} max={63} value={options.crf} onChange={(e) => update("crf", e.target.value)} />
          </Field>
          <Field label="编码速度预设">
            <SelectInput value={options.preset} onChange={(e) => update("preset", e.target.value)}>
              {["ultrafast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow"].map((preset) => <option value={preset} key={preset}>{preset}</option>)}
            </SelectInput>
          </Field>
          <Field label="编码 Profile" hint="例如 high、main、main10">
            <TextInput value={options.videoProfile} onChange={(e) => update("videoProfile", e.target.value)} />
          </Field>
          <Field label="像素格式" hint="例如 yuv420p、yuv422p10le">
            <SelectInput value={options.pixelFormat} onChange={(e) => update("pixelFormat", e.target.value)}>
          ...
          <Field label="输出帧率" hint="例如 23.976、25、30、60；留空保持">
            <TextInput value={options.frameRate} onChange={(e) => update("frameRate", e.target.value)} />
          </Field>
          <Field label="宽度" hint="只填一边时自动保持比例">
            <TextInput type="number" min={2} value={options.width} onChange={(e) => update("width", e.target.value)} placeholder="1920" />
          </Field>
          <Field label="高度">
            <TextInput type="number" min={2} value={options.height} onChange={(e) => update("height", e.target.value)} placeholder="1080" />
          </Field>
          <Field label="缩放算法">
            <SelectInput value={options.scalingAlgorithm} onChange={(e) => update("scalingAlgorithm", e.target.value as MediaOptions["scalingAlgorithm"])}>
          ...
          <Field label="裁剪表达式" hint="宽:高:x:y，例如 1920:800:0:140">
            <TextInput value={options.crop} onChange={(e) => update("crop", e.target.value)} />
          </Field>
          <Field label="显示宽高比" hint="例如 16/9、4/3">
            <TextInput value={options.aspectRatio} onChange={(e) => update("aspectRatio", e.target.value)} />
          </Field>
          <Field label="旋转">
            <SelectInput value={options.rotation} onChange={(e) => update("rotation", e.target.value as MediaOptions["rotation"])}>
            ...
            <div className="toggle-grid">
              <Toggle checked={options.flipHorizontal} onChange={(v) => update("flipHorizontal", v)} label="水平翻转" />
              <Toggle checked={options.flipVertical} onChange={(v) => update("flipVertical", v)} label="垂直翻转" />
              <Toggle checked={options.deinterlace} onChange={(v) => update("deinterlace", v)} label="YADIF 去隔行" />
            </div>
          </section>
          )}
          ...
          <Field label="音频编码">
            <SelectInput
              ...
              onChange={(event) => update("audioCodec", event.target.value as MediaOptions["audioCodec"])}
            >
              <option value="copy">复制原音频流</option>
              {ffmpegEncoders.includes("pcm_s24le") && <option value="pcm_s24le">PCM 24-bit</option>}
              {ffmpegEncoders.includes("pcm_s16le") && <option value="pcm_s16le">PCM 16-bit</option>}
              {ffmpegEncoders.includes("aac") && <option value="aac">AAC</option>}
              {ffmpegEncoders.includes("libmp3lame") && <option value="libmp3lame">MP3</option>}
              {ffmpegEncoders.includes("flac") && <option value="flac">FLAC</option>}
              {ffmpegEncoders.includes("libopus") && <option value="libopus">Opus</option>}
          ...
          <Field label="音频码率" hint="例如 192k、320k">
            <TextInput value={options.audioBitrate} onChange={(e) => update("audioBitrate", e.target.value)} />
          </Field>
          <Field label="采样率" hint="例如 44100、48000、96000">
            <TextInput value={options.sampleRate} onChange={(e) => update("sampleRate", e.target.value)} />
          </Field>
          <Field label="声道数" hint="1 单声道、2 立体声、6 为 5.1">
            <TextInput type="number" min={1} max={16} value={options.channels} onChange={(e) => update("channels", e.target.value)} />
          </Field>
          <Field label="音量" hint="例如 1.2、-3dB；留空保持">
            <TextInput value={options.volume} onChange={(e) => update("volume", e.target.value)} />
          </Field>
            </div>
            <div className="toggle-grid">
              <Toggle checked={options.loudnessNormalization} onChange={(v) => update("loudnessNormalization", v)} label="响度标准化（-16 LUFS）" />
            </div>
          </section>
          )}
          ...
          {options.operation === "gif" && (
            <section className="parameter-section">
              <h3>GIF 参数</h3>
              <div className="form-grid">
                <Field label="GIF 帧率"><TextInput type="number" min={1} max={60} value={options.gifFps} onChange={(e) => update("gifFps", Number(e.target.value))} /></Field>
                <Field label="GIF 宽度"><TextInput type="number" min={64} value={options.gifWidth} onChange={(e) => update("gifWidth", Number(e.target.value))} /></Field>
              </div>
            </section>
          )}
          </>
        ...
        <div className="toggle-grid">
          {(pageMode === "media" || options.operation === "remux") && (
            <Toggle checked={options.mapAll} onChange={(v) => update("mapAll", v)} label="保留全部媒体流" />
          )}
          <Toggle checked={options.preserveMetadata} onChange={(v) => update("preserveMetadata", v)} label="保留元数据与章节" />
          <Toggle checked={options.overwrite} onChange={(v) => update("overwrite", v)} label="覆盖已有文件" />
          {options.operation !== "pr-compatible" && (
            <Toggle checked={options.fastStart} onChange={(v) => update("fastStart", v)} label="网页快速播放（faststart）" />
          )}
        </div>
        <button
      ...
      {inspection && (
        <section className="inspection-card">
          <header>
            <strong>媒体信息</strong>
            <button type="button" onClick={() => setInspection(null)}><X size={14} /></button>
          </header>
