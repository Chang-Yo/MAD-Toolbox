export function LicensesPage() {
  return (
    <div className="page">
      <div className="page-title">
        <div>
          <span className="eyebrow">OPEN SOURCE</span>
          <h1>开源软件许可</h1>
          <p>MAD Toolbox 会保留所有内置项目的原始许可证、版权信息与源码地址。</p>
        </div>
      </div>
      <div className="license-list">
        <section>
          <strong>BBDown</strong>
          <span>MIT License · Copyright © 2020 nilaoda</span>
          <p>当前项目已归档。MAD Toolbox 分发的是固定、经过校验的非官方内置副本。</p>
        </section>
        <section>
          <strong>FFmpeg</strong>
          <span>8.1.2 · LGPL-2.1-or-later</span>
          <p>Full 版未启用 GPL/nonfree 组件，并随应用附带精确源码、构建资料和 LGPL 文本；Windows 版使用 BtbN 的 MIT 构建脚本。</p>
        </section>
        <section>
          <strong>yt-dlp / yt-dlp-ejs</strong>
          <span>2026.07.04 · Unlicense，发行程序同时包含其他第三方许可</span>
        </section>
        <section>
          <strong>MediaInfo</strong>
          <span>26.05 · BSD-2-Clause 形式许可及可选替代许可</span>
          <p>Windows 版同时附带 MediaInfo 官方 CLI 所需的 libcurl DLL，并保留 curl 许可文本。</p>
        </section>
        <section>
          <strong>Deno</strong>
          <span>2.9.4 · MIT License 及第三方组件许可</span>
        </section>
        <section>
          <strong>musicdl（可选外部依赖，不随应用分发）</strong>
          <span>Copyright © 2018–2026 CharlesPikachu · PolyForm Noncommercial 1.0.0</span>
          <p>MAD Toolbox 只调用用户自行安装的 musicdl，不复制或捆绑其软件；仅可按其许可证和各音乐平台条款使用。</p>
        </section>
      </div>
    </div>
  );
}
